...
pub fn init(event_loop: &mut EventLoop<Smallvil>) -> std::io::Result<SocketGuard> {
    ...
    // `Smallvil::arm_ipc_flush_timer`, only when a subscriber's write
    // genuinely didn't fully drain. This timer used to re-arm itself
    // unconditionally forever once started, waking every frame interval
    // even with zero subscribers connected -- the common case for most
    // of a session.
