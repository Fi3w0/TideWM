...
### Added
- Continuous lateral "swim" between workspaces (`src/swim.rs`), spatial-roadmap S0's minimal scoping: a horizontal trackpad swipe pans the viewport continuously instead of the ordinary one-shot discrete switch, while workspace identity itself stays exactly what it already was, a `u32` number owned by `Layouts::active`. This sidesteps S0's open per-spot-vs-shared-depth/named-anchors/multi-monitor questions (AGENT.md's spatial-model roadmap) entirely rather than resolving them -- there is no new data model here, only a purely-visual camera offset layered on top of the existing discrete trees. `SwimCamera` (`src/swim.rs`) is the same shape as `sway`/`viscosity`: a closed-form drag/spring record with no per-frame integrator, folded into the shared `window_visual_sample` aggregator so surfaces, popups, borders, shadows, glass, and every capture path move together. A drag past the half-spot mark advances the real workspace live (`switch_workspace_immediate`, not `switch_workspace` -- swim already owns the outgoing/incoming visual, so the wave-transition capture would be redundant and would fight the camera over what the next frame shows); release springs the residual offset back to zero over `snap_duration_ms`. The reserved scratchpad workspace 0 and `u32` overflow get the same boundary guard `workspace_swipe_target` (the existing discrete-swipe resolver) already established, mirrored in a new `swim_advance_target`; a step the boundary refuses is folded back into the camera (`cancel_advance`) instead of silently vanishing, so pressing against either edge reads as resistance. Wired through the existing `wp-pointer-gestures` compositor-consumed swipe path (`workspace_swipe_fingers`), decided once at gesture-begin so a config change mid-drag can't switch which model an already-running gesture follows; also gated on `!action_match` there, mirroring the existing End-handler precedence where a bound `swipe_left/right/up/down` action always wins over the workspace fallback -- without that gate, swim at the same finger count as a bound swipe action would have silently swallowed it. `swim { enabled, neighbors, response, snap_duration_ms }` configures it (`water_effects` remains the master bypass); config reload snaps any in-flight camera to rest when swim gets disabled out from under a drag. **Known, deliberate gap:** `neighbors` (how many adjacent spots stay mapped so they can slide into view) is parsed but not yet consumed -- this increment only translates the *current* workspace's own windows, so panning reveals wallpaper/blank on the entering side rather than live neighbor content until the crossing snaps the view over. Making neighbor spots actually visible mid-pan is real future scope, not a bug in this slice. Gesture events (`GestureSwipeBegin/Update/End`) are udev/real-libinput-touchpad-only -- confirmed from this codebase's own existing comment on the surrounding handler -- so, like `wp-pointer-constraints`, the drag-driven path is structurally impossible to live-test under the nested winit backend; only the pure `SwimCamera`/`swim_advance_target` math (17 unit tests: drag/wrap/spring/at-rest/cancel-advance and the boundary-clamp cases) and the non-gesture paths are verified. A nested smoke test confirmed the `window_visual_sample` signature change (now takes the output name, needed to look up the right camera) didn't regress its four call sites: default swim-off rendering, workspace switching, and floating/pseudo-tile toggling all still work, and a live config reload flipping `swim.enabled` on and off didn't crash or misbehave.
...
### Fixed
- Setting `$mod` away from Super (e.g. `$mod = ALT`, to free the key up for clients) gave no indication that Super was still functionally bound: `RawConfig::default()`'s keybinds are still spelled out literally as `Super+...` and merge in under any combo the user's config doesn't happen to redefine under the exact same string. A config warning now fires whenever `$mod` resolves away from Super but `pointer_modifier` still resolves to it. For keybinds specifically, the conflict is now resolved outright rather than just flagged: when a key is rebound under the new `$mod`, its untouched literal `Super+<key>` default is dropped, so the old binding stops firing alongside the new one. A `Super+X` the user wrote out themselves (a deliberate dual-bind) and any default never rebound via `$mod` are both left alone. Found on real AMD hardware.
