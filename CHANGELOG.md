...
### Added
- Captured wave workspace transitions (`src/workspace_transition.rs`), the third visible piece of Phase R1. A user workspace action now leaves the outgoing desktop on screen long enough to capture it after the submitted frame, applies the real workspace swap, then peels that texture away over the live incoming workspace through a direction-aware sinusoidal boundary (520ms by default). Forward and backward switches reveal from opposite edges; compositor chrome remains live above the desktop transition. Protocol-driven toplevel activation retains an immediate internal switch path so focus can still move to a hidden surface in the same dispatch. The transient state is bounded to one pending target and at most one ARGB8888 full-output texture per output (about 7.9MiB at 1080p or 31.6MiB at 4K): newer requests replace rather than queue, and completion, lock entry, output removal, or another switch releases the old texture. `water_effects = false` bypasses capture and animation completely. Winit, udev, and the separate screenshot render path all carry the transition element. A new global `workspace_transition { }` block makes the effect independently adjustable: `enabled`, `duration_ms`, and five easing curves follow niri's current per-animation configuration shape, while `wave_amplitude`, `wave_frequency`, and `edge_width` tune TideWM's shader-specific geometry. Hot reload applies tuning to the next transition; disabling the block clears active/pending transition state without suppressing water-glass or ripples. Verified live in a release nested session in both directions with mid-animation `grim` captures; the isolated `water_effects = false` session reported the new workspace immediately after the IPC action, and ten completed transition cycles settled within 0.5MiB PSS of the pre-cycle reading rather than growing per switch. Unit tests guard the shader contract, config parsing/defaults, and texture-cost arithmetic.
