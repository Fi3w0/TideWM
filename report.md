...
- Current implementation head: `eb9107b`
- Current TideWM version on the branch: `0.90.61`
- Push status: local only; nothing from this branch has been pushed.
- This branch was forked from master at `d9f5fcc` (2026-08-08) and has not been rebased or merged since; master has moved on independently (render/visual-identity work) and is now well ahead. Reconciling the two is future work, not done as part of closing findings here.
...
- Medium still open or awaiting a fresh audit: M-31 and M-37 through M-74. (M-03 fixed 2026-08-13 in `7f97c04`; M-11 fixed 2026-08-09 in `441d559`; M-24 fixed 2026-08-13 in `eb9107b` — see their finding bodies.)
- Performance opportunities P-01 through P-10: not worked in this branch unless a closed correctness fix incidentally reduced the same cost. Treat all ten as open until measured and re-audited. P-11 has code fixes in `1089450` and `0f74459` but still needs real-DRM measurement. P-12 has a code fix in `1089450` and awaits real-DRM verification. A 2026-08-09 real-hardware remeasurement on master `93106c4` / 0.90.58 (real Renoir DRM, not nested) confirmed P-11's RAM axis is fixed (PSS 65-71 MB, ~half Hyprland's 147 MB) but its GPU idle floor is not (still ~10 pp above Hyprland, sample not P-11-strict); it also surfaced a new P-13: VRAM hits 90% of the 512 MiB iGPU with nine glass windows. Maintainer direction: RAM is done, GPU (P-11) and VRAM (P-13) are the remaining attention items. A follow-up Hyprland head-to-head on the same machine/session (2026-08-09, Hyprland 0.56.2, blur off) revised both: P-11's idle GPU gap is effectively closed (TideWM ~32% vs Hyprland 28%, ~4 pp — not ~10 pp); and P-13's TideWM-specific attribution was wrong (Hyprland-plain hit 473 MiB VRAM at nine windows vs TideWM-glass 464 MiB, so the iGPU ceiling is shared client-buffer baseline, not backdrop capture). The water identity's real cost is now isolated as a new P-14: ~23 pp extra GPU-busy under per-window glass load (56% vs 33%). Standing maintainer direction: regardless of the favorable comparison, lower both GPU (P-14: damage-driven backdrop capture is the lead) and VRAM (P-13: reduce TideWM's own texture footprint, since client-surface cost is not controllable). **P-14 code fix landed 2026-08-09 in `441d559` (`ai/codex/report-fixes`): persisted per-surface `OutputDamageTracker` + reused texture + buffer age 1 (was a fresh tracker + age 0 forcing a full offscene render per glass window per frame), glass-layer commit is now a rendered-value fingerprint instead of a per-frame increment (static frost and settled glass stop forcing visible redraws; ambient/reactive tails still animate), and the floating-window and layer-shell capture passes build their behind-element list once per output instead of once per glass window. Nested boot verified; **real-AMD retest done 2026-08-10 (master `c2f1320` / 0.90.59): nine frost-kitty windows, 25 clean detached 1s samples -- gpu_busy min 26 / med 28 / avg 27.6%, down from 0.90.58's 56% and ~5 pp below same-machine Hyprland-plain 33%; PSS 62.9 MB, VRAM 455/512 MiB. Idle one-frost-window reads ~36% but that is driven by the config's always-on animated border + caustics (strict empty-workspace floor ~15%), not glass -- water-off and border-animate-off both left it at ~40%, and the idle > load inversion is itself proof the glass layer no longer pumps frames. P-14 closed.**
...
| Findings | Commit | Resolution |
| --- | --- | --- |
| M-24 | `eb9107b` | `pipewire_thread::start`'s five-second startup timeout no longer drops the worker `JoinHandle` (which only detaches the thread, not stops it). The join is now handed to a throwaway reaper thread that blocks on it and logs the outcome, bounding the leak to one idle thread per timeout instead of an invisible, permanently untracked one. Real thread cancellation isn't available in Rust; this makes the leak observable and bounded rather than eliminating the underlying stuck-library-call scenario, which is inherent to threading a blocking C API. |
...
**Fixed 2026-08-13, `eb9107b` (`ai/codex/report-fixes`).** The timeout branch of `start()` now spawns a small reaper thread that owns the worker `JoinHandle`, blocks on `worker.join()`, and logs whether it eventually exited cleanly or panicked. This does not make a wedged PipeWire call return any faster -- there is no portable way to cancel a native thread from Rust/std, and this project's own module doc comment deliberately isolates PipeWire in its own thread rather than driving it from the compositor's event loop the way niri does (`MainLoopRc` integrated into calloop via a `Generic` FD source, `niri/src/screencasting/pw_utils.rs`, read for comparison -- adopting that model here would mean removing the dedicated worker thread entirely, a materially bigger change than this finding's severity warrants). What the fix does close: the thread and its cloned `compositor` sender are no longer detached completely untracked on timeout; at worst there is one additional idle thread blocked in `join()` per timed-out attempt, and its eventual outcome is logged instead of silently lost. `cargo check --all-features`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo fmt --all -- --check`, and `cargo test --all-features --all-targets` (414 compositor tests, 9 `wavefmt` tests) all passed. No new unit test: the behavior under test is a real OS thread's timeout race, which the existing test suite doesn't spin up PipeWire for at all. Real-hardware verification (actually forcing a PipeWire startup timeout, e.g. by pointing at a wedged/nonexistent socket, and confirming the reaper's log line appears) remains pending.
