...
| Findings | Commit | Resolution |
| --- | --- | --- |
| M-47 | `2ebf8c9` | Every `sync_tide` pass now overwrites `tide.workspace`: Classic exposes its existing first-live-output number, while Ocean and zero-output sessions expose the documented `0` sentinel. This removes stale disconnected-output state and dormant Classic bookkeeping from Ocean's live Lua facts. |
| M-45 | `f3eff65` | Wave resolution returns the authoritative source graph, retaining lexical missing paths and canonical symlink targets. The watcher uses exact path-filtered non-recursive directory registrations for external includes and handles create, modify, atomic replacement, and removal; successful reloads replace the graph without a watch gap, while invalid candidates retain the active graph. Live out-of-tree modify/delete/recreate validation passed. |
...
| --- | --- | --- |
| P-01 | `248c2d3`, `b6d829a` | Retiling first stopped recomputing the same tiled world inside one state update. Each visible output render or desktop capture now builds one immutable placement snapshot and shares it across backdrop capture, layer blur, tab strips, glass/depth assembly, and final composition. Ocean rejects off-camera reefs before walking their BSP trees while retaining an actively dragged tile's owner; whole-world minimap/compass consumers are unchanged. The frame-owned snapshot avoids persistent invalidation state and makes no hardware assumptions. Closed. |
...
| P-12 | `1089450` | Caustics now requests redraws from a per-output deadline derived from the configured effective FPS, and its phase advances only when that deadline is due. This repairs the client-commit-dependent starvation and prevents unrelated animation from overdriving caustics. **2026-08-13 live Renoir/udev A/B:** with focused-border animation disabled and caustics configured at 24 fps, TideWM used 8.35% CPU and the whole-GPU counter stayed mostly 57-60%; with both sources damage-driven/static, TideWM fell to 2.57% CPU and whole-GPU busy to 25-28%. This confirms that the real DRM wake path is active and that `fps = 0` can be more expensive when another effect continuously redraws (caustics then piggybacks at that cadence), but it does not count presented caustics frames, so exact real-DRM cadence verification remains pending. |
| P-13 | `d6a2ff0`, `57552cd`, `662b542`, `332cf1c` | **Corrected after the 2026-08-09 Hyprland head-to-head.** Nine-window pressure is primarily the shared client-buffer baseline, not a TideWM regression. TideWM's own captures are measurable, explicitly downscalable, evicted when rules stop consuming them, and now released after their last presenting output drops them (including hidden/off-camera, DPMS, disconnect, and lock paths); the built-in wallpaper is optional. The general low-end-iGPU condition remains, while the concrete Tide-owned retention work is complete. See finding body. |
...
- Live standalone NVIDIA pass on installed release 0.90.106 (`13b104e`, `fiw/testing-fixes` on top of `5c63b90`; RTX 3060, proprietary 595.99.02 with the open kernel modules, Gentoo, kernel 7.2.7; udev, logged in from the display manager; HDMI-A-2 2560×1440@144 at 1.25× and DP-4 1920×1080@180 at 1.0× as reported by IPC; `gpu { render = vendor:nvidia; exclude = [vendor:amd] }` on a machine that also has a Radeon iGPU). This is the first standalone DRM run on Nvidia. `doctor` was all-pass. Full-output and cursor-overlay `grim` captures succeeded per output and for the combined 4960×1440 desktop. Unknown actions, an invalid mixed batch (nothing executed), an eval syntax error and a runaway `while true` eval (stopped by the execution budget) all failed cleanly; a 2 MB request was dropped at the size cap, random bytes got an `invalid request` error, and a truncated request got an error once EOF handling landed (`e4867b1`). 100 one-shot queries plus 20 interrupted subscribers left PSS unchanged (187,957→187,957 KiB) and descriptors flat (97→97). Measurements (20 untouched 1 Hz samples each; VRAM from `nvidia-smi -q -d PIDS`, GPU busy from `nvidia-smi`, which reads high at the 210 MHz idle clock and is not comparable to amdgpu `gpu_busy_percent`): idle with one window ~1% of one core, PSS 179.5 MiB, 131 MiB TideWM VRAM, 309 MiB card total; nine frost-glass Kitty windows plus one ~0-1% CPU, 139 MiB TideWM VRAM (card 653 MiB, the rest is client buffers as on AMD), 4.6 MiB of backdrop texture; back to idle 133 MiB. An earlier run on `5c63b90` with larger tiles measured 156→196 MiB TideWM VRAM for the same nine windows (15.3 MiB of backdrops).
  Memory: PSS rises once to a plateau and stays there. `smaps` splits it into ~75-79 MiB heap and ~60-95 MiB of `/dev/nvidia*` mappings (driver-owned GPU buffers mapped into the process, which amdgpu does not show as PSS). Three cycles of opening and closing eight frost windows returned to exactly the same PSS with a flat heap, so this is a high-water mark, not a leak. The doctor's decorated-tier soft limit (280 MiB) was reached after about 4 hours of use on the older build; on Nvidia the tier limits should probably count the driver mappings separately.
  Found and fixed on the same branch, each with regression tests: client drag and drop never worked (`WaylandDndGrabHandler` was an empty impl, so Smithay cancelled every source; `2727de9`); glass and other captures drew the wallpaper at `1/scale` size on fractional outputs (`3286393`); `output { }` blocks were ignored on reload (`8ebb259`); `spawn` could not pass arguments containing spaces and tidectl consumed flags after the command (`6517964`); `cursor_hide_after = 0` was rejected (`182df1b`); doctor mislabeled card nodes (`490df73`). Automatic attention depth became opt-in (`c593da7`).
  Still open from this pass: user-reported tearing on the HDMI output only, unchanged at scale 1.0, not reproduced as a TideWM fault (cable or HDMI bandwidth at 1440p144 suspected); PDF text selection in Firefox reported as improved but still imprecise; layer glass and layer shaders were not exercised (no layer-shell client running yet).
  Screencast: portal screen sharing from native Vesktop (vesktop-bin 1.6.7, Electron on the Wayland ozone backend) through xdg-desktop-portal to the TideWM ScreenCast backend and PipeWire 1.6.8 worked on this Nvidia setup, confirmed visually by the maintainer; the portal reported `AvailableSourceTypes = 7`. A successful session leaves no log line: `src/screencast` only logs failures, so a share is invisible in the session log unless it goes wrong.

- Live standalone AMD pass on installed release 0.90.101 (`384fe98`, Renoir, udev; all geometry and cadence read from IPC): `doctor --json` was all-pass with the expected TideWM portal, PipeWire, xwayland-satellite, and AMD render node. Full-output `grim` captures verified the normal desktop, an in-flight water transition, five-window BSP/master/cascade layouts, pseudo-tiling, floating conversion, invalid-config diagnostics/recovery, and an XWayland GLX client. Twenty rapid workspace reversals settled on workspace 1 with no transition texture retained; the original window remained tiled, focused, and non-fullscreen. Four disposable translucent Kitty clients mapped/unmapped cleanly; five-window frost/water measured 64.6 MiB PSS with 0.9 MiB of active backdrop texture. A 15-second static A/B disabling only 24 fps caustics and focused-border animation changed `animation_active` from true to false and compositor CPU from 8.30% to 1.55%; the three touched Wave files were restored byte-for-byte. An invalid Wave edit kept the previous live config, showed the exact file/line in the first-party overlay, and cleared after restoration. Two identical rounds of 700 one-shot queries plus 100 interrupted subscribers converged to the same 91,006 KiB PSS high-water mark, zero swap, and exactly 93 descriptors, showing bounded first-use/cache growth rather than continued leakage. `glxgears` through xwayland-satellite mapped, rendered at 58.53/58.32 fps against the live 60 Hz mode, and unmapped cleanly. No new TideWM panic/error or coredump appeared. P-12's real-DRM wake path is reconfirmed, but this stripped release exposes neither a caustics-present counter nor a usable external perf/BPF/AMD busy counter, so exact presented-caustics cadence remains pending.

- Live standalone AMD pass on installed release 0.90.72 (`dfc00b7`, Renoir, udev, 1920×1080@60 as reported by IPC; no geometry or mode was supplied by the test): full-output `grim` and `grim -c` captures succeeded; a reversible workspace 1→2→1 switch emitted the expected focus/workspace sequence and left the original window state intact; unknown actions, an invalid mixed batch, and invalid Wave eval all failed cleanly; 100 one-shot queries plus 20 interrupted subscribers left PSS flat at 88,840→88,838 KiB, swap at zero, and descriptors non-growing (91→90). Post-test doctor remained all-pass with no TideWM panic/error or coredump. The live log exposed two config-lowering warnings (`workspace_motion_delay = 0ms`, bracket-list ripple triggers) and `tidectl --socket … report` exposed a global-argument parsing bug; 0.90.101 fixes all three with exact regression tests and an end-to-end release-`tidectl` report run. The original live config values were restored after the performance A/B, and workspace 1 was restored after transition testing.

- After `2ebf8c9`, strict all-target/all-feature Clippy, formatting, diff checks, all 456 compositor tests, all 8 `tidectl` tests, all 9 `wavefmt` tests, and the locked all-feature release build passed. Live nested Ocean validation returned `0` for `tide.workspace` while the same output IPC record returned `active_workspace: null`; the disposable compositor and test config were cleaned up. Covers M-47.

- After `f3eff65`, strict all-target/all-feature Clippy, formatting, diff checks, all 455 compositor tests, all 8 `tidectl` tests, all 9 `wavefmt` tests, and the locked all-feature release build passed. Live nested release validation proved an absolute include outside the main config directory hot-reloaded on modification, dropped its Lua value on deletion, and recovered it on recreation; normal shutdown removed the disposable socket and all test files. Covers M-45.

- After `332cf1c`, strict all-target/all-feature Clippy, formatting, diff checks, and the locked all-feature release build passed. The effective result is 452 compositor tests plus 3 `tidectl` and 9 `wavefmt` tests; the sandbox allowed 449 compositor tests and the three IPC Unix-socket tests passed with normal socket permissions. Release-GLES nested validation on the test output's live 945×1018 geometry measured one visible frost capture at 3,570,552 bytes/count 1, zero bytes/count 0 after moving the window to a hidden workspace, and exactly one same-sized capture after visiting it. Covers TideWM's P-13 retention work.

- After `b6d829a`, strict all-target/all-feature Clippy, formatting, diff checks, and the locked all-feature release build passed. The effective all-feature result is 451 compositor tests plus 3 `tidectl` and 9 `wavefmt` tests; the sandbox allowed 448 compositor tests and the three IPC Unix-socket tests passed when rerun with normal socket permissions. Covers P-01.
...
**Fixed in two passes on 2026-08-13: `911b942`, completed by `5fe5db4` (`ai/codex/report-fixes`).** The first pass made the known consumers saturating. The follow-up closes its recorded residual with a hardware-independent invariant: every output rectangle edge and the complete desktop span must fit Smithay's real `i32` logical-coordinate domain, with logical sizes derived from each live mode, transform, and fractional scale. This is not a fixed desktop-size cap. Startup/hotplug preserves representable configured positions and falls back to live-geometry automatic placement with a warning when one does not fit. Runtime output-management validates the complete proposed transaction before mutation and replies with the configuration-level `failed` event when it does not fit. The earlier note was correct that `zwlr_output_configuration_head_v1` has no `invalid_position` field error, but incorrect that this prevented clean rejection of the complete configuration. Exact old-to-new output translation is checked in `i64`, and the remaining floating restore-rectangle clamp now also uses `i64` intermediates. Niri's `src/niri.rs::reposition_outputs` was read directly: it derives size from the live output and falls back when requested rectangles overlap. Hyprland's `MonitorLayoutController::checkOverlapsAndNotify` warns about overlap after arranging. TideWM keeps representable overlap valid and combines the new domain invariant with U-16's deterministic ownership. Strict Clippy and formatting passed; the effective all-feature test result is 448 compositor tests plus 3 `tidectl` and 9 `wavefmt` tests, with the three IPC socket tests rerun successfully outside the syscall sandbox; the all-feature release build passed.
...
**Fixed 2026-08-13, `bc38869` (`ai/codex/report-fixes`).** IPC snapshots and the `windows`/`focused-window` queries now resolve surfaces through `Smallvil::mapped_toplevel_window`, the existing protocol-mapped lookup shared with XDG lifecycle handling. That lookup covers Ocean, hidden Classic BSP trees, floating tags, parked group members, the Classic Depth Deck, and visible Space fallback without admitting `unmapped_toplevels`. The complete query enumerates the foreign-toplevel numeric-id registry in deterministic id order rather than treating the currently rendered `Space` as lifecycle state; XDG unmap still removes the foreign entry, so truly unmapped clients remain absent. No second registry, polling loop, output property, or hardware constant was introduced. Strict Clippy, formatting, and diff checks passed. The effective all-feature result is 452 compositor tests plus 3 `tidectl` and 9 `wavefmt` tests; the sandbox allowed 449 compositor tests and all 11 IPC tests passed with normal Unix-socket permissions. The locked all-feature release build passed. Live release-GLES nested validation mapped one Kitty as `window_id: 1` on workspace 1, moved it to inactive workspace 2 while workspace 1 stayed active, and confirmed `tidectl windows --json` still returned the same id with `focused: false` and `workspace: 2`; both disposable processes exited cleanly.
...
**Fixed for every catchable termination signal 2026-08-13, `ef6cf33` (`ai/codex/report-fixes`).** The calloop signal source now handles SIGINT, SIGTERM, and SIGHUP alongside SIGCHLD and stops `state.loop_signal`, so normal Rust teardown drops `SocketGuard` and unlinks the exact PID-scoped socket. SIGKILL remains inherently uncatchable; PID-scoped discovery and the already-fixed conservative stale-socket path remain the correct fallback after a kill or process/machine failure. Direct reading of niri's `src/utils/signals.rs` exposed the necessary companion fix: calloop's signalfd is close-on-exec but its blocked signal mask is inherited. TideWM now routes every compositor-owned external exec through one pre-exec command builder that clears the mask, covering startup/action spawns, both real session-environment runner calls, and xwayland-satellite probe/launch. The hook calls only libc signal-mask operations over stack state. A regression test blocks SIGTERM in its parent thread, spawns a self-terminating shell through the builder, restores the parent's mask, and confirms the child dies from SIGTERM instead of ignoring it. Strict all-target/all-feature Clippy, formatting, and diff checks passed. The effective all-feature result is 453 compositor tests plus 8 `tidectl` and 9 `wavefmt` tests; the sandbox allowed 450 compositor tests and all 11 IPC tests passed with normal Unix-socket permissions. The locked all-feature release build passed. Live nested release validation sent SIGTERM to the exact disposable compositor PID 214639: TideWM logged the handled signal, exited with status 0, removed `/run/user/1000/tidewm-214639.sock`, and left neither itself nor its xwayland-satellite child running.
...
**Fixed 2026-08-13, `52136de` (`ai/codex/report-fixes`).** Every `tidectl` connection now has a bounded connect phase despite `std::os::unix::net::UnixStream` lacking a connect-timeout API: only the blocking connect runs on a short-lived worker while the CLI waits at most 10 seconds, and every caller exits on timeout so the worker cannot outlive the command. One-shot request writes and reads have the same deadline and retain at most 16 MiB. Subscription request/ack I/O is deadline-bound and both the ack and later newline-delimited records are capped at 256 KiB; after a valid ack the read deadline is deliberately cleared so a healthy bar can wait indefinitely between real events. `doctor`/`report` compositor queries reuse this path instead of their prior unbounded duplicate, while every external diagnostic helper gets a 10-second process deadline and retains at most 8 MiB of stdout while continuing to drain excess output until exit. These are protocol/resource bounds, not monitor, refresh, scale, output-count, or GPU assumptions. Direct reference reading found Hyprland's `hyprctl/src/main.cpp::request` sets a five-second receive timeout but appends replies without a size cap, while niri's `niri-ipc/src/socket.rs` uses blocking unbounded `read_line`; neither streaming contract was copied blindly. Three pure record/response-bound tests and two subprocess cap/deadline tests were added. Strict all-target/all-feature Clippy, formatting, and diff checks passed. The effective all-feature result is 452 compositor tests plus 8 `tidectl` and 9 `wavefmt` tests; the sandbox allowed 449 compositor tests and all 11 IPC tests passed with normal Unix-socket permissions. The locked all-feature release build passed. Live release-GLES validation completed a subscription handshake, kept the stream open, received the exact later `workspace-changed` record, and completed all ten `tidectl doctor` checks through the bounded paths; all disposable processes exited cleanly.

### M-45 — Config watcher misses external includes and deletion events

**Confidence: confirmed, narrower than the original wording.** The current watcher recursively covers `.wave` files below the main config directory, so ordinary sibling and nested includes are already handled. Wave also supports absolute, `~`, parent, and symlinked includes outside that tree, however, and those targets do not hot-reload. The filter handles modify/create but not removal, so deleting an included `.wave` can leave the old config silently active until another event.

**Fixed 2026-08-13, `f3eff65` (`ai/codex/report-fixes`).** The authoritative Wave evaluation pass now returns its complete source graph: the main file, every reached include's absolute lexical path (retained even when missing), and every available canonical target (so symlink destinations remain observable). `ConfigWatcher` owns exact path-filtered, non-recursive directory registrations rather than assuming the graph is below the main config directory. Direct parents catch write, remove, and atomic-replacement events; an existing ancestor preserves recovery when a target directory is absent or removed. A successful transactional reload registers the new graph before retiring obsolete directories, while a failed candidate keeps both the active Lua/config and previous watch graph. This follows the useful source-graph/mtime model in niri's `src/utils/watcher.rs`, adapted to TideWM's event-driven notify/debounce design. Two regression tests cover external missing/symlink graph retention and removal/missing-ancestor relevance. Strict all-target/all-feature Clippy, formatting, and diff checks passed; all 455 compositor, 8 `tidectl`, and 9 `wavefmt` tests passed, as did the locked all-feature release build. Live nested release validation loaded an include outside the main config directory and observed its IPC-evaluated value change from `"one"` to `"two"`, become `null` after deletion, and return as `"three"` after recreation. The disposable compositor exited normally, removed its IPC socket, and all temporary files were deleted. No output geometry, refresh rate, GPU, input-device, or other hardware/configurable value was hardcoded.
...
**Fixed 2026-08-13, `2ebf8c9` (`ai/codex/report-fixes`).** `sync_tide` now assigns the workspace on every refresh instead of conditionally leaving the previous value behind. Classic keeps the established first-live-output numeric contract; Ocean and a zero-output session publish `0`, the sentinel already documented by `TideInfo`, because neither owns a meaningful numbered active workspace. This mirrors niri's underlying invariant that `active_workspace()` is optional when its monitor set is not normal, while preserving TideWM's intentionally different Ocean model and Lua number type. A pure regression test covers Classic with/without an output and Ocean with/without dormant Classic state. Strict all-target/all-feature Clippy, formatting, and diff checks passed; all 456 compositor, 8 `tidectl`, and 9 `wavefmt` tests passed, as did the locked all-feature release build. Live nested Ocean validation returned `0` from `tidectl eval tide.workspace --json` while `tidectl outputs --json` reported `active_workspace: null` for the same live output. No output, cadence, GPU, or device property was hardcoded.
...
**Fixed 2026-08-14.** Re-audited first: caustics (`caustics_redraw_delay`) and compass (`compass_frame_elements`) already checked lock state independently, but `has_active_animation()` -- the single predicate the udev maintenance timer, per-CRTC dirty re-arm, and winit redraw request all consume -- did not. Animated borders, ambient physics, glass, swim cameras, and ocean camera motion all kept the fast timer alive while locked. Fix: a lock-state gate at the return of `has_active_animation()` (after all retain/prune calls, so cleanup still runs): `if !matches!(self.session_lock, SessionLock::Unlocked) { return false; }`. Additionally, `update_float_physics_full`'s unconditional `request_redraw()` (line 1913) is now lock-gated so wave-driven physics bodies that never self-prune can't keep requesting one render per slow-timer tick while invisible. The DPMS-off case was already partly handled by the udev timer filtering `powered_off` surfaces for its interval, and individual effect predicates that check lock state also cover their own DPMS interaction. Live-verified in a nested release session with animated borders on a focused kitty window: `animation_active: true`, CPU 4.44% before lock; `animation_active: false`, CPU 0.57% while locked via `hyprlock` (custom config avoiding the nested-only screencopy crash). Resume-on-unlock is logically guaranteed (the gate passes through when `SessionLock` returns to `Unlocked`) but could not be exercised live because killing the lock client triggers the fail-closed security termination. All 461 compositor tests pass; strict Clippy and fmt clean.
...
**Fixed 2026-08-14.** Re-audited against current code first: the finding's line numbers were stale but the bug itself stood exactly as described -- `swap_workspaces` (`state.rs`) updated fullscreen entries, maximized entries, and floating tags, and still never touched `groups`. The four consumers confirmed live in current code: `tab_strip_elements`'s Classic `group.output != output_name` skip (strip vanishes on the new output and finds no placement on the old), `ungroup`'s `layout.insert(&group.output, group.workspace, ...)` reinsertion target, `activate_toplevel`'s group-owner hidden-workspace resolution, and the accessibility snapshot's owner filter. Fix: after `layout.swap_active`, groups whose owner matches either side of the swap are retagged to the other output's active workspace via a new pure helper `swapped_group_owner` (same extract-the-tricky-part pattern `group_removal_outcome` already uses), gated on the Classic engine -- under Ocean `swap_active` moves nothing and the fields are migration cache, so they must not be touched. Groups parked on hidden workspaces keep their owner since their tree never moved. Unit test pins both swap directions, hidden-workspace exclusion, and the same-workspace-number case. Full suite (461 compositor tests), strict Clippy, and fmt on the touched file passed. Live cross-output verification remains pending real hardware: nested winit is structurally single-output (`winit.rs:73`, `output_count` is always 1 after the winit 0.30 `EventLoop` restriction was confirmed empirically), so the `swap-workspaces` action itself stays in the same "unverified cross-output" bucket it was already documented in -- this fix neither worsens nor closes that.
...
**Fixed 2026-08-14.** Re-audited against current code first: the finding stood exactly as described. `map_toplevel` inserted into a hidden workspace's tree, `retile()` mapped only the active tree into `Space`, then `toggle_floating` needed `space.elements()` and silently returned -- so `rule.float` no-op'd (window stayed tiled), `rule.pin` inserted into `self.pinned` over a still-tiled window (corrupt state), and `rule.maximize` found a tiled window via `do_maximize_request`'s early tiled-return (never maximized). Fix: a new Space-free `float_hidden_tiled_window` converts a just-mapped hidden-tree window to floating using `Layouts`'s own window handle and tile rect, tagging the float with the *rule-targeted* workspace instead of the output's active one. `map_toplevel` picks this path when the Classic target workspace differs from the active one. `apply_floating_placement` was generalized to fall back to the floating tag's rect when the window isn't in `Space`, and its `space.map_element` is now visibility-gated so applying a rule position/size to a hidden float doesn't accidentally make it visible. A pinned hidden-workspace window maps immediately into `Space` at a bounds-clamped rect, since pinning promises all-workspace visibility; without that, a pinned window aimed at a hidden workspace would stay invisible forever (pinned windows are exempt from the hide/show cycle, so nothing would ever map it). `rule.fullscreen` was confirmed out of scope: `do_fullscreen_request` is already Space-independent (uses `toplevel_window`'s comprehensive lookup and keys its entry off the surface), and retile's fullscreen override reconciles it on switch-in. Live-verified in a nested release session (single winit output, workspace 1 active) with three real `kitty --class` clients: `workspace=2 float=true` floated on hidden ws 2 and appeared on switch-in; `workspace=2 pin=true` floated, pinned, and showed immediately on ws 1 (pinned visibility); `workspace=2 maximize=true` floated and maximized on hidden ws 2, correctly hidden on ws 1, maximized on switch-in. Switch-to-ws-1 then switch-back-to-ws-2 confirmed correct hide/show for all three. All 461 compositor tests pass; strict Clippy and fmt clean on the touched files.
...
**Partially fixed 2026-08-13, `5fe5db4`.** The position half and any scale that produces non-positive, non-finite, or unrepresentable logical geometry are now rejected by the shared complete-desktop validator before output mutation. An unusually high positive scale that still resolves to a representable one-pixel logical output remains accepted. Whether TideWM should impose a usability minimum on that user-configurable value is a separate policy decision, so M-72 stays open for its scale half rather than smuggling in an arbitrary hardware assumption under U-15.
...
- **P-01 — Ocean world layout is recomputed repeatedly.** `state.rs:7104-7137` computes tiled layouts twice, and `ocean.rs:1375-1438` rebuilds the whole world again per output per frame, including off-camera reefs. Cache topology/world rectangles and cull before layout/render.
  ...
  **Fixed 2026-08-13, `248c2d3` and `b6d829a` (`ai/codex/report-fixes`).** The first change reused one tiled-world result inside Ocean retiling. The follow-up removed the broader repeated render work without introducing a long-lived cache: each backend output render and each desktop capture builds one frame-owned `Vec<PlacedWindow>`, then passes that immutable snapshot through window/layer backdrop capture, tab-strip assembly, depth/glass construction, and visible composition. A glass-enabled Ocean output therefore builds its placement scene once rather than up to four times in one pass. `OceanSpace::placements` also filters reefs against the live camera before invoking their BSP layouts. Every supported layout and preserved-size attachment stays inside its reef; the active tiled-drag owner is retained explicitly because its temporary drag rectangle can leave that reef. Complete-world consumers such as minimap/compass still use the uncropped world path. No output size, resolution, refresh rate, connector count, or hardware tier is assumed. P-01 closed.
...
- **P-13 — VRAM scales with glass-window count and approaches the iGPU ceiling under normal use; needs a budget, not yet investigated in code.** Confidence: measured live on real AMD Renoir/amdgpu (512 MiB VRAM) at master `93106c4` / 0.90.58. Steady-state `mem_info_vram_used`: ~299 MiB at one frost window, ~349 MiB idle with caustics, and **464/512 MiB (90%) with nine glass windows** (frost/water-glass backdrop capture active). Per-glass-window cost is roughly 13 MiB (the per-window offscreen backdrop-capture texture), consistent with the architecture's stated ~31 MiB wallpaper backing. With a GPU-accelerated browser or more frosted windows on top, the Renoir iGPU enters its shared-system-memory fallback (no hard fail, but degraded throughput). PSS is unaffected (glass lives in VRAM, not process memory), so this is invisible to RAM monitoring — which is why P-11's RAM picture looks clean while VRAM does not. No code read yet; needs a confirmed cap or eviction policy for backdrop-capture textures (bounded count, shared/reused capture, or an LRU drop under VRAM pressure) and a measurement on a discrete GPU with more headroom before deciding whether an explicit budget is warranted. Flagged by the maintainer this session as a remaining attention item alongside P-11's GPU floor.
  ...
  **Tide-owned retention completed 2026-08-13, `332cf1c` (`ai/codex/report-fixes`).** Earlier commits made capture payload visible in `tidectl perf` (`520a3e4`), added configurable 1–4× linear downscaling (`d6a2ff0`), released captures after a rule stopped consuming them (`57552cd`), and made the 4K fallback wallpaper optional (`662b542`). The remaining leak-shaped case was visibility: a mapped glass window hidden on a Classic workspace or outside every Ocean camera kept its full texture. Each capture now tracks the names of outputs whose latest scene still presents it. Per-output render reconciliation, DPMS-off, connector removal, and session lock remove those claims; the texture drops after the last claim, while one window visible through two Ocean cameras survives until both views release it. Names avoid retaining Smithay `Output` objects. Release-GLES nested verification on the live 945×1018 test output measured 3,570,552 backdrop bytes/count 1 while visible, zero bytes/count 0 after moving the window to a hidden workspace, and one same-sized capture when visiting that workspace. No VRAM capacity, mode, scale, refresh, output count, or timeout is guessed. The original system-wide iGPU pressure remains a client-buffer/hardware reality; a shared-per-output blur texture is still a possible visual/performance tradeoff, not required to bound TideWM's retained state.
...
- **U-04 — Fixed: Ocean directional focus travels to off-camera windows.** Commit `1b14b48` compares candidates in the current output camera's projected coordinate space. A visible neighbor focuses without camera movement; an off-camera neighbor focuses and smoothly centers the camera using the user's existing `ocean.camera_animation_ms` and `camera_sway`. Screen-pinned windows participate only at their real viewport placement, fullscreen content owned by another output is excluded, and Classic keeps screen-geometry navigation. This follows niri's animated column activation and Hyprland's scrolling-layout follow-focus behavior after reading both implementations directly. The score uses `f64` centers so valid extreme `i32` world coordinates cannot overflow, and the shared path no longer raises tiled targets above floating windows.
...
- **U-10 — Fixed: one capture client could monopolize the global 64-slot queue.** Commit `51c476e` records the Wayland owner of ext-image-copy and wlr-screencopy requests, limits one client to eight pending frames, and discards its queued work immediately on disconnect. Multi-output admission also derives an output share from the current live output count, with the existing four-render batch as its floor, while a single-output session retains the full global capacity. Internal PipeWire frames keep their one-pending-frame-per-stream behavior and remain covered by the output/global limits. Niri's actual screencopy code isolates queues per manager; TideWM retains its shared renderer queue and applies ownership at admission because both Wayland capture protocols and PipeWire deliberately converge there. These are bounded protocol-work limits, independent of resolution, refresh rate, connector identity, or GPU capacity.
...
- **U-15 — Fixed: output placement is bounded as one logical desktop.** Commit `5fe5db4` derives each proposed output rectangle from its live mode, transform, and scale, then checks every edge, the whole-desktop span, and live translation deltas against Smithay's actual `i32` logical-coordinate domain before mutation. Udev config falls back with a warning; `wlr-output-management` fails an invalid transaction atomically. Representable negative coordinates and overlap remain valid. No connector, position, resolution, refresh rate, scale, monitor count, or GPU capacity is built in.
- **U-16 — Fixed: overlapping outputs had nondeterministic point ownership.** Commit `e6bf868` routes all point-to-output decisions through one selector and orders every matching live output by its runtime name. Surface, layer, lock, Ocean window, focus, drag, and placement paths therefore agree even though Smithay's `Space::output_under` iterator is hash-backed. Niri currently takes the first Smithay result, while the inspected Hyprland code reports overlap as an invalid layout; TideWM preserves user-supplied overlapping coordinates and defines stable ownership instead of rewriting the layout or pretending that coordinate overlap is content mirroring. No connector identity, geometry, mode, scale, or refresh value is built in.
