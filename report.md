...
- Current implementation head: `0f74459`
- Current TideWM version on the branch: `0.90.42`
...
- After `0f74459`, `cargo test --locked --all-features` passed all 365 compositor tests and all 9 `wavefmt` tests outside the restricted IPC socket sandbox. Strict `cargo clippy --locked --all-targets --all-features -- -D warnings` and `cargo fmt --all -- --check` also passed.
...
- Strict all-target/all-feature Clippy has passed through the current `0.90.42` implementation head.
