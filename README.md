...
- **Two spatial engines.** [Classic and Ocean now have a fixed design](SPATIAL_MODEL.md). Classic keeps real workspaces; its opt-in per-workspace Depth Deck supports direct workspace-like Down/Up switching, manual park, full-deck navigation, exact-slot swap recall, a distinct vertical pressure-wave cue, and safe hot-disable restore. The model-neutral placement boundary is now complete: Classic produces one renderer-facing scene consumed by both backends, captures, glass/depth effects, and grouped tab strips. Ocean is the next structural step, a workspace-free infinite 2D canvas where X is travel and Y is physical depth, implemented as a second placement producer rather than a second renderer.
