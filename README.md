...
> [!NOTE]
> TideWM already works as a real Wayland compositor: tiling, multi-monitor, workspaces, IPC, and most of the protocol surface a daily driver needs are all in. The water/aqua render work is underway: water-glass and configurable impulse ripples have landed, with workspace waves and depth/buoyancy next. Screen sharing (PipeWire/xdg-desktop-portal) works end to end on a real standalone session now, verified through both OBS and Discord. This is exactly the stage where testing on hardware I don't own is most useful, jump into [Discord](https://discord.gg/ZhkxA83cKk) if something breaks.
...
- Water-glass refraction for translucent floating windows and configurable impulse ripples on map, focus, or urgent attention
...
- **Finish the R1 identity slice.** Water-glass and impulse ripples are built. Wave-based workspace transitions and the first depth/buoyancy model are next.
