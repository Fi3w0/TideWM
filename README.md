...
> [!NOTE]
> TideWM already works as a real Wayland compositor: tiling, multi-monitor, workspaces, IPC, and most of the protocol surface a daily driver needs are all in. The full water/aqua identity is built now too: impulse ripples, wave workspace transitions, water-glass and per-app frosted glass, analytical shadows, rounded borders, configurable window animations, liquid move/resize viscosity, connected-vessel BSP resizing, opt-in floating sway, ambient caustics, and automatic window depth/buoyancy. Ocean, the second spatial engine (a zoomable continuous 2D canvas instead of workspaces), is live-verified on real AMD hardware, standalone udev/DRM session, not just nested, complete with an off-screen compass, a whole-world overview minimap, and a live hot-reload migration between it and Classic with no restart. Floating windows can also bob and drift like they're sitting on water, with an optional full rigid-body tier (mass, collisions, a traveling wave field). An IPC event-stream lets bars and widgets subscribe to live state instead of polling. Screen sharing (PipeWire/xdg-desktop-portal) works end to end on a real standalone session, verified through both OBS and Discord. This is exactly the stage where testing on hardware I don't own is most useful, jump into [Discord](https://discord.gg/ZhkxA83cKk) if something breaks.
...
- Two spatial engines, chosen with `spatial_engine = classic|ocean` and switchable live on a hot reload (every window migrates in place, no restart): Classic keeps real numbered workspaces; Ocean drops workspaces for one continuous zoomable 2D canvas with local tiling reefs, independent per-output pan/zoom cameras, physical sink/dredge/surface depth, and named/numeric bookmarks. Ocean windows can also detach straight out of a reef into a free zoom-aware drag (`freeform_windows`), while smart tiling can swap tiled windows and reattach floaters with their own sizes. Dragging empty canvas pans the camera directly. A bioluminescent edge-glow compass points toward off-screen urgent/deep windows, and holding the minimap key peeks a whole-world overview to click-travel anywhere. [Full design in SPATIAL_MODEL.md](SPATIAL_MODEL.md).
...
- Selectable water-glass refraction or per-app adjustable frosted glass, rounded client clipping, animated active/inactive/urgent gradient borders, analytical drop shadows, configurable open/close/move window animations, liquid move/resize viscosity, connected-vessel BSP resizing, opt-in floating-window sway, configurable impulse ripples, full-screen water-wave workspace transitions, ambient caustics, automatic visual depth/buoyancy, and an independent opt-in Classic Depth Deck for tiled-window park/swap recall
- Cosmetic floating-window ocean physics (`float_physics { tier = light|full }`): windows bob and drift when disturbed by a drag, a map, or a workspace transition; `full` adds real mass, inter-window collisions, and a continuous traveling wave field
...
- **Continuous swim navigation** (`swim { }`): adjacent workspace content is revealed continuously without making hidden windows focusable or IPC-visible. Verified on a real touchpad (3-finger swipe pans continuously and settles); nested winit cannot emit the required gesture events, so that's the only way to exercise it.
...
- **Water/decoration effects**: the full identity and decoration set (ripples, wave transitions, water-glass, frost, shadows, borders, window animations, viscosity, connected-vessel resize, sway, depth/buoyancy, caustics) is live-verified nested and has also passed a real standalone udev/DRM hardware pass on AMD. Floating-window ocean physics (`light`/`full` tiers) is nested-verified only so far; its own standalone hardware pass is still open.
- **Ocean compass and overview**: both live-verified nested (off-screen urgent/deep glow cues, hold-to-peek whole-world minimap with click-to-travel and decorative presets). Real-hardware pass still open.
...
- **Feel-tuning.** Viscosity, sway, depth timings, cascade's drag feel, floating-window ocean physics, and the transition/ripple presets ship with working defaults; the actual feel still gets refined against real use. The water/decoration stack (ripples, wave transition, frost, shadows, rounding, borders, window animations) and swim's real-touchpad path all passed their standalone udev/DRM hardware pass on AMD in 0.90.x.
- **Real hardware for what's still nested-only.** The Ocean compass/overview and both floating-window ocean physics tiers (`light`/`full`) are nested-verified only; a standalone udev/DRM pass for each is still open.
